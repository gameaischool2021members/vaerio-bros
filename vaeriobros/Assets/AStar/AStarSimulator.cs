...
public class AStarSimulator : MonoBehaviour
{
    ...
    List<int[]> visitedStates = new List<int[]>(); // the closed-list of A*
	...
	{
        // TODO what of this do we still need?
		...
		public float simulatePos()
		{
			...
			parentPos.sceneSnapshot = outer.backupState(); // TODO copy the parents scene since we will change this scene
			...
			remainingTime = calcRemainingTime(outer.levelScene.plumberXposition, outer.levelScene.plumberXacceleration)
				 + (outer.getMarioDamage() - initialDamage) * (1000000 - 100 * timeElapsed);
			if (isInVisitedList)
        ...
        public List<bool[]> createPossibleActions(SearchNode currentPos)
        {
            ...
            // jump
            // if (canJumpHigher(currentPos, true)) possibleActions.Add(createAction(false, false, true));
            possibleActions.Add(createAction(false, false, true));

            // run right
            possibleActions.Add(createAction(false, true, false));
            // if (canJumpHigher(currentPos, true)) possibleActions.Add(createAction(false, true, true));
            possibleActions.Add(createAction(false, true, true));

            // run left
            possibleActions.Add(createAction(true, false, false));
            // if (canJumpHigher(currentPos, true)) possibleActions.Add(createAction(true, false, true));
	...
	private static bool[] createAction(bool left, bool right, bool jump)
	{
    ...
    public int getMarioDamage()
    {
        ...
        if (levelScene.isGap(levelScene.plumberXposition) &&
                levelScene.plumberYposition > levelScene.gapHeight(levelScene.plumberXposition)) // What is gapHeight
        {
        ...
        //&& (System.currentTimeMillis() - startTime < Math.min(200,timeBudget/2))) <- this makes the game a bit more jerky, but allows a deeper search in tough situations
        {
            ...
            if (realRemainingTime < 0)
            {
                ...
                continue;
                    ...
                    && isInVisited((int)current.sceneSnapshot.plumberXposition, (int)current.sceneSnapshot.plumberYposition, current.timeElapsed))
            {
    ...
    private bool isInVisited(int x, int y, int t)
    {
        ...
        int xDiff = 2;
        int yDiff = 2;
        foreach (int[] v in  visitedStates)
        {
